...
const renderContentType = (data, item) => {
  let returnedContent;

  typeof item.content === 'function'
    ? (returnedContent = item.content(data[item.match]))
  ...
  return content.map((item, index) => {
    return (
      <div key={`${item.match}-${index}`} className={item.classname}>
        {renderContentType(data, item)}
      </div>
    );
  });
};

const CardBuilder = ({ data, cardClass, cardHeader, cardBody, cardFooter }) => {
  return (
    <Card className={cardClass}>
      ...
      <CardFooter className={cardFooter.classname}>
        {renderContent(data, cardFooter.contents)}
      </CardFooter>
    </Card>
  );
};
